...
class Sprite {
  ...
  Sprite(this.width, this.height, this.numColors, this.paletteData, this.pixelData);
  ...
  Uint8List pack() {
    ...
    switch (numColors) {
      case <= 2:
        bpp = 1;
        packed = pack1Bit(pixelData);
        break;
      case <= 4:
        bpp = 2;
        packed = pack2Bit(pixelData);
        break;
      case <= 16:
        bpp = 4;
        packed = pack4Bit(pixelData);
        break;
      default:
    ...
    Uint8List payload = Uint8List.fromList(List.filled(6 + numColors * 3 + packed.length, 0));
